...
# ── 성장 플리: 선곡/회전(매시간 믹스) ──
LOCK_HOURS      = 4        # eta 이 시간 이내면 '코앞' → 달성까지 고정(쿨다운 무시)
...
IMMINENT_PING_HOURS = 4      # 이 시간마다 새 알림 (3이면 3시간마다)
# D-Day 알림
...
CAFE_POST_ENABLED  = False         # [설정] 전체 on/off (끄면 아무것도 안 함)
...
CAFE_POST_DELAY  = 30            # [설정] 카페 글 사이 간격(초) — 연속등록 방지
